...
public class BuildingPlacement : MonoBehaviour
{
    ...
    {
        var funcStartTime = Time.realtimeSinceStartup;
        var mouseRay = cam.ScreenPointToRay(mousePosition);
        if (Physics.Raycast(mouseRay, out var hit))
        {
            if (hit.collider.TryGetComponent<BuildingGround>(out _))
            {
                ...
                {
                    Debug.Log($"Grid is empty at area: {string.Join(',',hitArea)}");
                    blockPreview.transform.position = Grid.GridCoordinatesToWorldPosition(hitGridCoordinates);
                    blockPreview.SetActive(true);
                    Debug.Log("Time to find hover position: " + (Time.realtimeSinceStartup - funcStartTime));
                    return;
                }
            }
        }
